...
_openclaw_onboard() {
  _arguments -C \
    ...
    "--mode[Wizard mode: local|remote]" \
    "--auth-choice[Auth: setup-token|token|chutes|openai-codex|openai-[REDACTED]
    ...
    "--cloudflare-ai-gateway-[REDACTED]
    "--moonshot-[REDACTED]
    "--kimi-code-[REDACTED]
    "--gemini-[REDACTED]
    "--zai-[REDACTED]
...
_openclaw_cron_add() {
  _arguments -C \
    ...
    "--channel[Delivery channel (last|whatsapp|googlechat|slack)]" \
...
_openclaw_pairing_list() {
  _arguments -C \
    "--channel[Channel (whatsapp, googlechat, slack)]" \
    "--json[Print JSON]"
}
